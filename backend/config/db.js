...
const createDatabase = async () => {
    try {
        ...
        const res = await client.query(`SELECT 1 FROM pg_database WHERE datname = '${process.env.DATABASE}'`);
        if (res.rowCount === 0) {
            await client.query(`CREATE DATABASE ${process.env.DATABASE}`);
            console.log(`Base de données '${process.env.DATABASE}' créée avec succès`);
        } else {
            console.log(`ℹ️ La base de données '${process.env.DATABASE}' existe déjà`);
        }
        await client.end();
    } catch (err) {
...
const insertRoles = async () => {
    if (await isTableEmpty("roles")) {
        ...
        console.log("ℹ️ La table `roles` contient déjà des données, pas d'insertion.");
    }
};
    ...
    if (await isTableEmpty("class")) {
        for (const key in classes) {
            ...
            for (const role of classes[key].roles) {
                const roleRes = await pool.query(`SELECT id FROM roles WHERE label = $1;`, [role]);
        ...
        console.log("ℹ️ La table `class` contient déjà des données, pas d'insertion.");
    }
};
